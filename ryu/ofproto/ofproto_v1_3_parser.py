...
# my code
@_set_msg_type(ofproto.OFPT_RULE_MOD)
class OFPRuleMod(MsgBase):
    """
        
    """
    def __init__(self, datapath, priority=0, flags=0,
                 command=ofproto.IPTABLES_SET_POLICY,
                 acttype=0, actdesc=0, match=None, prop=None):
        super(OFPRuleMod, self).__init__(datapath)
        self.priority = priority
        self.flags = flags
        self.command = command
        self.acttype = acttype
        self.actdesc = actdesc
        ...
        if prop is None:
            prop = OFPMatch()
        assert isinstance(prop, OFPMatch)
        self.prop = prop

    def _serialize_body(self):
        msg_pack_into(ofproto.OFP_RULE_MOD_PACK_STR0, self.buf,
                      ofproto.OFP_HEADER_SIZE,
                      self.priority, self.flags, self.command,
                      self.acttype, self.actdesc)

        offset = (ofproto.OFP_RULE_MOD_SIZE -
                  ofproto.OFP_MATCH_SIZE)

        match_len = self.match.serialize(self.buf, offset)
        offset += match_len

        prop_len = self.prop.serialize(self.buf, offset)
        offset += prop_len

    # def __init__(self, datapath, priority, op, config):
    #     super(OFPRuleMod, self).__init__(datapath)
    #     self.priority = priority
    #     self.op = op
    #     self.config = config
    ...
    #     msg_pack_into(ofproto.OFP_RULE_MOD_PACK_STR, self.buf, offset,
    #                   self.priority, self.op, self.config)

    #     offset = ofproto.OFP_RULE_MOD_SIZE

@_register_parser
@_set_msg_type(ofproto.OFPT_RULE_MOD_REPLY)
class OFPRuleModReply(MsgBase):
    """
    """
    def __init__(self, datapath, priority=None, op=None, config=None):
        super(OFPRuleModReply, self).__init__(datapath)
        self.priority = priority
        self.op = op
        self.config = config
    ...
    @classmethod
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
        msg = super(OFPRuleModReply, cls).parser(datapath, version, msg_type,
                                                   msg_len, xid, buf)
        msg.priority, msg.op, msg.config = struct.unpack_from(
            ofproto.OFP_RULE_MOD_REPLY_PACK_STR, msg.buf,
            ofproto.OFP_HEADER_SIZE)
        print msg.priority, msg.op, msg.config
        return msg
