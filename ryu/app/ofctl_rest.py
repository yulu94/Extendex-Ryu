...
class StatsController(ControllerBase):
    ...
    # my code
    def mod_rule_entry(self, req, cmd, **_kwargs):
        try:
            flow = ast.literal_eval(req.body)
        ...
        if cmd == 'add':
            cmd = dp.ofproto.OFPRC_ADD
        elif cmd == 'modify':
            cmd = dp.ofproto.OFPRC_MODIFY
        elif cmd == 'delete':
            cmd = dp.ofproto.OFPRC_DELETE
        else:
            return Response(status=404)
        ...
        _ofctl = supported_ofctl.get(_ofp_version, None)
        if _ofctl is not None:
            _ofctl.mod_rule_entry(dp, flow, cmd)
        else:
            LOG.debug('Unsupported OF protocol')
            return Response(status=501)
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # my code
        # rule mod api 
        uri = path + '/ruleentry/{cmd}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='mod_rule_entry',
                       conditions=dict(method=['POST']))
